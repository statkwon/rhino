#EDA

library(tidyverse)
set.seed(519)
gas <- read.csv("data/cleansing/gasstation.csv")

#변수명 변경
colnames(gas) <- c('gu', 'gas_station', 'car', 'station_per_car', 'inflow', 'outflow', 'land_value', 'university',
                   'enterprise', 'distributor', 'parking_area', 'school', 'road_area', 'population', 'day_pop', 'night_pop')
library(tidymodels)

#전처리 시작
gas_tbl <- gas %>% as_tibble %>% 
  janitor::clean_names()

gas_tbl <- gas_tbl %>%
  mutate(car_per_station = car/gas_station)

skimr::skim(gas_tbl)
colnames(gas_tbl)

# 강북구 imputation (유일하게 inflow, outflow가 NA이다.)
gas_tbl_lm <- gas_tbl %>% 
  filter(gu != '강북구') %>% 
  select(-gu)

inflow.lm <- lm(inflow~road_area, data=gas_tbl_lm) #road_area와 교통량(inflow, outflow) 상관관계 높아서 유일변수로 회귀분석 실시
outflow.lm <- lm(outflow~road_area, data=gas_tbl_lm)
gas_tbl$inflow[3] <- predict(inflow.lm, newdata = gas_tbl %>% filter(gu == '강북구') %>% select(-gu))
gas_tbl$outflow[3] <- predict(outflow.lm, newdata = gas_tbl %>% filter(gu == '강북구') %>% select(-gu))

# Scaling
gas_tbl_z <- gas_tbl %>%
  mutate_each(funs(z=scale(.)), -c('gu','station_per_car')) %>% #standardization
  select(ends_with('z'))

# 단순 EDA
hist(gas_tbl_z$gas_station_z, freq=T)

# 시각화 (분석)
library(ggplot2)
#car_per_station ~ outflow 
ggplot(gas_tbl, aes(x=outflow, y=car_per_station)) +
  geom_point() +
  geom_smooth(method='lm', color='red')

ggplot(gas_tbl, aes(x=inflow, y=car_per_station)) +
  geom_point() +
  geom_smooth(method='lm', color='red')

ggplot(gas_tbl, aes(x=day_pop, y=car_per_station)) +
  geom_point() +
  geom_smooth(method='lm', color='red')

ggplot(gas_tbl, aes(x=land_value, y=car_per_station)) +
  geom_point() +
  geom_smooth(method='lm', color='red')

ggplot(gas_tbl, aes(x=enterprise, y=car_per_station)) +
  geom_point() +
  geom_smooth(method='lm', color='red') 

gas_tbl %>%
  ggplot(aes(fill=gu)) +
  geom_bar(aes(x=gu, y=inflow), stat='identity') +
  coord_flip() +
  theme(legend.position = 'none')

ggplot(gas_tbl, aes(x=gu, y=outflow, fill=gu), stat='identity') +
  geom_bar()
  


# ggplot(gas_tbl, aes(x=inflow, y=station_per_car)) +
#   geom_point() +
#   geom_smooth(method='lm')
# 
# ggplot(gas_tbl, aes(x=outflow, y=station_per_car)) +
#   geom_point() +
#   geom_smooth(method='lm')


