                     ...
                     tabItems(
                       tabItem(tabName = 'feature_graph', h2('구별 특성 비교'), 
                               fluidRow(
                                 ...
                                 infoBoxOutput('min'),
                                 box(plotOutput('graph', width = 1200, height = 600))
                               )),
                       ...
                       tabItem(tabName = 'station_map',
                               fluidRow(
                                 box(title='수소차수 지도',
                                     collapsible=TRUE,leafletOutput('map2'), width = 8, height = '100%'),
                                 box(title='수소차수 테이블',
                                     collapsible=TRUE,tableOutput('table2'), width = 3, height = '100%')))
                     )
)
...
server = function(input, output) {
  ...
  output$graph = renderPlot({
    data %>%
      ggplot(aes(fill = 구)) + geom_bar(aes(x = 구,y = get(input$features1)), stat='identity') + 
  ...
  output$xselect = renderUI({
    selectInput("xselect", "Options", unique(loc_data[,1]), selected = '종로구', selectize = TRUE)
  })
  output$xselect2 = renderUI({
    selectInput("xselect2", "Options", loc_data[,2][which(loc_data[,1]==input$xselect)], selected = '청운동', selectize = TRUE)
  })
}
