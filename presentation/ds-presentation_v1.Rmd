...
: 자치구별 등록차량, 대학교, 기업체, 우통업체, 주차장


[한국 감정원](http://www.r-one.co.kr/rone/resis/statistics/statisticsViewer.do?menuId=LFR_12200)
: 공시지가 

[교육 통계 서비스](https://kess.kedi.re.kr/mobile/search?searchTxt=%EC%84%9C%EC%9A%B8%EC%8B%9C%20%ED%96%89%EC%A0%95%EA%B5%AC%EC%97%AD%EB%B3%84%20%ED%95%99%EA%B5%90%EC%88%98)
: 학교 수 
...
- 법정동 데이터를 이용 : 더 많은 변수 사용을 위해 구 단위로 데이터 통합
...
y_mean <- mean(data_model$Hyd_car)
y_std <- sqrt(var(data_model$Hyd_car))
...
simul_result <- data.frame(gu =car_gu$gu,
                           ...
                           before_pred = round(predict(modFit_repeatedcv, data),3), 
                           after_pred = round(predict(modFit_repeatedcv, data_simul),3))
